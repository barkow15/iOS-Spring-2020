...
//  MediaCaptureDemo
//
//  Created by Philip Thomas Barkow on 27/03/2020.
...
class ViewController: UIViewController, UINavigationControllerDelegate, UIImagePickerControllerDelegate {
    ...
    @IBAction func photosBtnPressed(_ sender: Any) {
        imagePicker.sourceType = .photoLibrary // this sets the source of the imagepicker to the photolibrary - this can also be set to camera
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        imagePicker.delegate = self // assign the object from this class to handle image picking return - THIS REFERENCES THE OBJECT
    }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let p = touches.first?.location(in: view) {
            startPoint = p.x // register position of finger
        }
    }
